...
class Renamer{
  constructor(){
     ...
     .messages .renamer-tag{margin-right:6px;}
		'</style>`;
  ...
  showTagModal(user){
    let { nick, username, colour ,tag } = ((user)=>user?user:{})(this.getNickname(user.id))
          ...
          <div class="modal-inner">
              <form class="form">
                  ...
                  <div class="form-inner">
                      ...
                      <div class="control-group">
                          <label class="reset-tag"><a>Delete Tag</a></label>
                      </div>
  					<div class="control-group">
      ...
      .appendTo("#app-mount>:first-child")
  	  .on("click",".reset-tag",(e)=>{this.resetUserProp(user.id,"tag"); this.process(true); modal.remove() })
      ...
      modal.find("#tagtext").click().focus();
  }
  showInputModal(user){
  ...
  process(force){
    ...
    $(".chat strong.user-name:not([renamer])").each((i,el) => {
      //this.log(i,el);
    ...
    $(".channel-members .member-username-inner:not([renamer]),.channel-voice-states .avatar-small+span:not([renamer])").each((i,el) => {
      el.setAttribute("renamer","");
