...
class Renamer{
  ...
  setSwatches(currentColor,colorOptions,parent){
	...
	this.log(selection)
    const swatches = $(`<div class="swatch large default${useDefault?' selected':''}" style="background-color: #7289da;"></div>
    <div class="swatch large custom${selection===-1&&!useDefault?' selected':''}" style="background-color: rgb(255, 255, 255);"></div>
    <div class="regulars">${ colorOptions.map((val,i)=>`<div class="swatch${i===selection?' selected':''}" style="background-color: ${val};"></div>`).join("")}</div>`)
    .appendTo(parent)
  ...
  process(force){
    ...
    $(".channel-members .member:not([renamer])").each((i,member) => {
      ...
      const userData = this.getNickname( this.getReactInstance( member )._currentElement.props.children[0].props.user.id );
      if (userData){
